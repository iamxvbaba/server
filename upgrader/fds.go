...
// ListenWithCallback 返回从父进程继承的侦听器，或调用提供的回调来创建一个新的侦听器。
//如果必须应用某些自定义来创建连接，则应使用此方法。请注意，callback时不得使用基础的 Fds 对象，因为它将在调用过程中被锁定。
func (f *Fds) ListenWithCallback(network, addr string,
	...
	if _, ok := ln.(Listener); !ok {
		ln.Close()
		return nil, fmt.Errorf("%T doesn't implement upgrader.Listener", ln)
	}
...
func (f *Fds) ListenPacketWithCallback(network, addr string, callback func(network, addr string) (net.PacketConn, error)) (net.PacketConn, error) {
	...
	if _, ok := conn.(PacketConn); !ok {
		return nil, fmt.Errorf("%T doesn't implement upgrader.PacketConn", conn)
	}
